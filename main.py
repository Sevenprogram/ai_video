...
# 口播文稿来源：ai_analyze | manual_storyboard | local_file
SPOKEN_SCRIPT_DIR = os.path.join(_VIDEO_MODULE_DIR, "video_shoot", "spoken_script")
...
def build_script_from_video_analysis(
    recording_path: str,
    duration_minutes: int = 3,
) -> str:
    """
    口播文稿来源选项 1：AI 分析录屏视频内容，自动生成口播文稿。
    提取若干关键帧，用视觉模型分析画面内容，再生成文稿。
    """
    try:
        import cv2
        from PIL import Image
    except ImportError as e:
        raise RuntimeError(f"需要 opencv-python 和 Pillow 才能分析视频内容: {e}")
    from module import jeniya_complete_only
    ...
    # 均匀取 6 帧
    indices = [int(i * total_frames / 6) for i in range(6)] if total_frames >= 6 else list(range(total_frames))
    frames_pil = []
    cap = cv2.VideoCapture(recording_path)
    for idx in indices:
    ...
    # 构造多模态消息（使用 OpenAI 格式）
    from openai import OpenAI
    from config import PROXY_API_KEY, PROXY_BASE_URL

    client = OpenAI(api_key=PROXY_API_KEY, base_url=PROXY_BASE_URL)

    analysis_prompt = """You are a professional video content analyst. Describe what you see in these video frames in detail:
- What websites, apps, or UI elements are visible?
- What data, charts, or content is displayed?
- What is the likely purpose/topic of this screen recording?
Output a concise English description (2-4 paragraphs) suitable for generating a narration script."""
    ...
    try:
        response = client.chat.completions.create(
            model="gemini-2.0-flash",  # Jeniya 中的模型
            messages=[{"role": "user", "content": content}],
            max_tokens=2048,
        )
        video_description = (response.choices[0].message.content or "").strip()
    except Exception as e:
        # 如果多模态失败，回退到纯文本分析
        raise RuntimeError(f"视觉模型调用失败: {e}")

    if not video_description:
        raise RuntimeError("AI 未能分析出视频内容")

    # 用描述生成口播文稿
    prompt_text = build_script_from_recording_prompt(
        prompt=video_description,
        duration_minutes=duration_minutes,
        reply="（AI 基于视频画面自动分析）",
    )
    return jeniya_complete_only(prompt_text)


def build_script_from_storyboard(storyboard_text: str, duration_minutes: int = 3) -> str:
    """
    口播文稿来源选项 2：用户输入分镜主要内容，AI 根据分镜生成口播文稿。
    """
    from module import gemini_complete_only
    prompt_text = build_script_from_recording_prompt(
        prompt=(storyboard_text or "").strip() or "（无分镜描述）",
        duration_minutes=duration_minutes,
        reply="",
    )
    return gemini_complete_only(prompt_text)


def load_script_from_local_file(filename: str) -> str:
    """
    口播文稿来源选项 3：从 spoken_script 目录读取本地保存的文稿文件。
    """
    path = os.path.join(SPOKEN_SCRIPT_DIR, filename)
    if not os.path.isfile(path):
        raise FileNotFoundError(f"文稿文件不存在：{filename}")
...
def _get_clips_from_folder(folder_name: str, min_clips: int = 3) -> List[str]:
    ...
    特殊处理 idle_hands_open.mp4：作为常规待机动作，会被重复使用更多次。
    """
    base = Path(_resolve_digital_human_dir()) / folder_name.strip()
    if not base.exists() or not base.is_dir():
        return []
    paths = []
    idle_path = None  # 记录 idle 动作路径
...
def _build_clip_list(shots: list) -> List[str]:
    ...
    for shot in shots:
        ...
        clips.append(target if os.path.exists(target) else (idle if idle_exists else target))

    # 至少保留 3 个片段
    while len(clips) < 3 and idle_exists:
        clips.append(idle)

    # 智能重复：idle 动作重复更多次，使视频更自然
    # 如果 idle 存在，将其均匀分布在整个片段列表中
    if idle_exists and len(clips) > 1:
        result = []
        idle_count = clips.count(idle)

        # 如果 idle 比例较低，增加一些 idle 片段
        if idle_count < len(clips) / 2:
            # 在每个非 idle 片段后插入一个 idle（最多每隔一个插入）
            for i, clip in enumerate(clips):
                result.append(clip)
                if clip != idle and i < len(clips) - 1:
                    result.append(idle)
            clips = result

    return [c for c in clips if os.path.exists(c) or c == idle]
...
def run_recording_full_pipeline(
    ...
    参数：
        prompt              录屏/视频描述（script_source=manual_storyboard 时作为分镜内容）
        ...
        log_fn              (step, msg) 回调
        outputs_base        输出根目录
        script_source       口播文稿来源：ai_analyze | manual_storyboard | local_file
        script_storyboard   script_source=manual_storyboard 时的分镜内容
        script_local_file   script_source=local_file 时的文件名（spoken_script 目录下）
    ...
    # Step 1：获取口播文稿（根据 script_source）
    if script_source == "local_file" and script_local_file:
        emit("script", f"从本地文件加载口播文稿：{script_local_file}")
        try:
            script = load_script_from_local_file(script_local_file)
        except Exception as e:
            emit("error", f"加载文稿失败: {e}")
            raise
    elif script_source == "ai_analyze":
        emit("script", f"AI 分析视频内容并生成口播文稿（目标 {duration_minutes} 分钟）...")
        try:
            script = build_script_from_video_analysis(recording_path, duration_minutes)
        except Exception as e:
            emit("error", f"视频分析/文稿生成失败: {e}")
            raise
    else:
        # manual_storyboard：用户输入分镜，AI 生成文稿
        storyboard_text = (script_storyboard or prompt or "").strip()
        if not storyboard_text:
            emit("error", "请输入分镜主要内容或选择其他文稿来源")
            raise ValueError("缺少分镜内容")
        emit("script", f"根据分镜内容生成口播文稿（目标 {duration_minutes} 分钟）...")
        try:
            script = build_script_from_storyboard(storyboard_text, duration_minutes)
        except Exception as e:
            emit("error", f"文稿生成失败: {e}")
            raise
    with open(script_path, "w", encoding="utf-8") as f:
