...
def generate_base_action_library(
    ...
    :param output_dir: 输出目录，例如 "video_module/action_clips/数字人A"
...
if __name__ == "__main__":
    ...
    2. 程序会提示输入新文件夹名称（例如：数字人A、客服1号等）

    3. 然后列出当前支持的动作 key，例如：
       - idle_hands_open
       - nod_light
       - look_side_screen
       - shrug_open_hands

    4. 在提示中输入想生成的动作 key，多项用英文逗号分隔：
       例如：idle_hands_open,nod_light

    若直接回车，不输入任何内容，则会生成所有预设动作。

    生成的视频会保存在 action_clips/你输入的文件夹名/ 目录下
    """
    import argparse
    ...
    # 1. 获取文件夹名称
    folder_name = input("请输入保存文件夹名称（例如：数字人A、客服1号，直接回车默认用时间戳）: ").strip()

    # 2. 确定最终输出目录
    if not folder_name:
        folder_name = datetime.now().strftime("%Y%m%d_%H%M%S")
        print(f"[INFO] 未输入名称，使用时间戳作为文件夹: {folder_name}")

    base_output_dir = os.path.join(os.path.dirname(__file__), "action_clips")
    ...
    if actions is None:
        ...
        line = input("\n请输入要生成的动作 key（多项用英文逗号分隔，直接回车表示全部）: ").strip()
        if line:
            actions = [s.strip() for s in line.split(",") if s.strip()]
        else:
            actions = None  # 表示全部

    print(f"\n[INFO] 视频将保存到: {output_dir}\n")
